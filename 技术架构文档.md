...
手表戴在腕上就能控：**前臂左 / 右旋** ↔ `left` / `right`，**手腕上抬 / 下压** ↔ `up` / `down`，**把表盘翻过去** ↔ `play` / `pause`。全部只用**原始加速度计 @50Hz**（重力方向由软件低通自估，系统的旋转矢量 / 重力 / 线性加速度等融合接口一概不用），零权限、零 root，**Bridge 协议与扩展一行未改**（复用既有动作名）。
...
> ⚠️ 上表列的是「**哪些输入可用**」，不是「实际用了哪些」。**真正进判定链路的只有"原始加速度计"这一个输入**：重力方向是软件低通自估的（§5.4.2），陀螺仪只存诊断值、旋转矢量 / 线性加速度 / 磁力计完全没碰。

#### 5.4.2 传感器与采样

- **只注册一个传感器：`TYPE_ACCELEROMETER`，50Hz**（`SENSOR_DELAY_GAME`）。
- **"重力"不是读系统的 `TYPE_GRAVITY`，是自己低通估出来的**：一阶低通（时间常数 `gravityTauSec = 0.06s`，`α = 1 − e^(−dt/τ)`）分离出慢变的重力分量，再归一化成重力单位向量 `u`。原始加速度 = 重力 + 手的线性加速度，线性加速度不参与角度计算。
- **系统的一切"封装接口"都不用**：`TYPE_ROTATION_VECTOR`（旋转矢量）、`TYPE_GRAVITY`、`TYPE_LINEAR_ACCELERATION`、`getRotationMatrix()`、四元数——全仓 **0 处引用**。从"原始三轴加速度"到"转了哪根轴、转了多少度"整条链路都是本项目自己的算法（§5.4.3）。
  为什么不用现成的融合结果：① 旋转矢量是系统按自己的手势语义调的，抬手看表这类姿态变化会被它优先当作"姿态/航向变化"吃掉或平滑掉，而我们要的恰恰是这类缓慢倾转；② 系统融合输出依赖磁力计，腕上磁场环境不可控；③ 全自研才可能做到"零 Android 依赖"，从而在 Mac 上秒级回归（§5.4.6）。
- **不积分**：识别只问"重力方向相对静止基准 `g0` 转了多少"，不做陀螺积分 ⇒ **没有累积漂移**。
- **陀螺仪也不参与判定**：`onGyro()` 只把模长存进 `gyroRaw` 供日志诊断，识别器里没有任何判据读它；正式路径干脆**不注册陀螺**（只有标定页会喂进来）。实测陀螺零偏恒定 59.5°/s，且同一姿态下 AC 供电与 USB 供电差 50 倍。
