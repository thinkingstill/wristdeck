...
| 5 | 手表上网方式：独立 Wi-Fi 还是手机蓝牙共享 | 连通性与延迟 | 独立 Wi-Fi（实测已连 <手表IP>） |
...
| 9 | 工程不是 git 仓库、无 `.gitignore`；`watch/local.properties` 含本机 sdk 绝对路径 | 无法追溯改动，机器相关文件容易被带出去 | `git init` + `.gitignore`（`local.properties`、`build/`、`.gradle/`、`node_modules/`） |
